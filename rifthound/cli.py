...
def main(argv=None):
 ...
 if ns.cmd=='hunt':
  ...
  if ns.reconner_output:ov=merge(ov,{'recon':{'reconner_output':ns.reconner_output}})
  if ns.bbot:ov=merge(ov,{'recon':{'bbot':{'enabled':True}}})
  if ns.dnsx:ov=merge(ov,{'recon':{'dnsx':{'enabled':True}}})
  if ns.waymore:ov=merge(ov,{'recon':{'waymore':{'enabled':True}}})
  if ns.arjun:ov=merge(ov,{'recon':{'arjun':{'enabled':True}}})
  if ns.jsattack:ov=merge(ov,{'recon':{'jsattack':{'enabled':True}}})
  if ns.service_correlation:ov=merge(ov,{'recon':{'service_correlation':{'enabled':True}}})
  if ns.nmap_vuln:ov=merge(ov,{'recon':{'service_correlation':{'enabled':True,'nmap_vuln':True}}})
  ...
  cfg=build_config(ns.preset,ns.config,ov);ui=UI(ns.quiet,ns.verbose);ui.banner(__version__);ph=[1,2,3,4] if ns.all else sorted(set(ns.phase)) if ns.phase else cfg['phases'];ui.info(f'preset={ns.preset} phases={ph} roots={cfg["scope"]["roots"]}');Pipeline(cfg,ui,ns.dry_run).run(ph,seeds,ns.ai);return 0
 return 0
